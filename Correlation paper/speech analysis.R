...
qids_id <- merge(qids, IDmap[,2:3], by = c("p_id"))
...
#SPEECH
speech_u <- fread("speech-Unscripted-25-09-2022.csv", data.table=F) %>%
  mutate(date_str = as.Date(date_str, tz = NULL, formats = NULL))

speech_s <- fread("speech-Scripted-25-09-2022.csv", data.table=F) %>%
  mutate(date_str = as.Date(date_str, tz = NULL, formats = NULL))
...
speech <- speech_s         #  <-------- change here from speech_s to speech_u
speech_merge <- merge(speech, IDmap[,2:3], by = "p_id", all = TRUE, incomparables = T)
...
names(speech_event)
speech_event$Task[1]
speech_s <- speech_event   #  <-------- change here from speech_s to speech_u
...
#merge speech and qids
m_speech <- merge(speech, qids_event, by = c("p_id", "event_name", "date_assessment" ))
m_speech <- m_speech %>% 
...
a <- as.data.frame((sapply(m_speech, function(x) sum(is.na(x))))/nrow(m_speech))
a <- as.data.frame(which(a > .5, arr.ind = TRUE))


names(speech)

other_vars <- c(1:3) # columns that are not predictor variables
...
inc_vars <- speech_s[, -c(other_vars)]
...
# --- correlation plot ----
m <- cor(speech_u, use = "complete.obs",
         method = "spearman")
...
d_speech <-merge(m_speech, demo)
names(d_speech)

## conduct the models adjusting for Age, gender, (and depression)

data = d_speech         #     <- - - - select the data to be used for the models
